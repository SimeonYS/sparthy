...
class SparthyPipeline:

    # Database setup
    conn = sqlite3.connect('sparthy.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `sparthy`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM sparthy WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO sparthy (date, title, link, content)"
